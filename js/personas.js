import { db } from "./firebase.js?v=20260429b75";
import { collection, addDoc, serverTimestamp, onSnapshot, query, orderBy, deleteDoc, doc, updateDoc } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
if(!canEdit){
  toast("Modo solo lectura: no podés modificar personas.", "err");
}
...
function render(){
  ...
  const rows = all.filter(p=>{
    const hit = (p.nombre||"").toLowerCase().includes(q) || (p.telefono||"").toLowerCase().includes(q);
  ...
  tbody.innerHTML = rows.map(p=>`
    <tr>
      <td><b>${p.nombre||""}</b><div class="small muted">${p.activo===false?'INACTIVO':''}</div></td>
      <td>${p.telefono||""}</td>
